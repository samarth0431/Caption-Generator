...
def transcribe_audio_wav2vec2(audio_file_path):
    processor, model = load_audio_model('facebook/wav2vec2-base-960h')
...
def transcribe_audio_wav2vec2(file_name):
    input_audio, sample_rate = librosa.load(file_name, sr=16000)
    processor, model = load_audio_model('facebook/wav2vec2-base-960h')

    inputs = processor(input_audio, return_tensors="pt" , sampling_rate=sample_rate, padding=True, normalization=True).input_values

    logits = model(inputs).logits
    transcription = decode_transcription(logits, processor)
    return transcription
